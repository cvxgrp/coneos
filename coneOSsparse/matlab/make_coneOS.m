...
%try
    ...
    cmd = sprintf ('%s ../direct/ldl.c %s ../direct/private.c LINKLIBS="\\$LINKLIBS -lm %s %s" -output coneos_direct', cmd, common_coneOS, LOCS, BLASLIB) ;
    eval(cmd);
    ...
    if (COMPILE_WITH_OPENMP)
        ...
        cmd = sprintf('mex -O %s CFLAGS="-std=c99 -O3 -pthread -DMATLAB_MEX_FILE %s %s" ../indirect/private.c %s -I../ %s -output coneos_indirect LINKLIBS="\\$LINKLIBS -lm  %s %s"',  arr, d, LCFLAG, common_coneOS, INCS, LOCS, BLASLIB);
    end
    eval(cmd);
...
%{
    ...
    if (COMPILE_WITH_OPENMP)
        ...
        cmd = sprintf('mex -O %s CFLAGS="-std=c99 -O3 -pthread -DMATLAB_MEX_FILE %s %s" ../indirect/private.c %s -I../ %s -output coneos_indirect LDFLAGS="\\$LDFLAGS -lm  %s %s"',  arr, d, LCFLAG, common_coneOS, INCS, LOCS, BLASLIB);
    end
    eval(cmd);
